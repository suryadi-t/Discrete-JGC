...
def poisson_hurdle_loss(y_true, y_pred):
    """Assumes both y_true and y_pred have 2 dimensions.
    y_true: [y,z]
        y: target value
        z: indicator variable (1 if y>0, 0 if y==0)
    y_pred: [lambda,mu]
        lambda: predicted mean for nonzero target value, i.e. mean for the zero-truncated Poisson distribution
        mu: predicted probability for zero    
    """    
    y = y_true[:,0]
    z = y_true[:,1]
    lam = y_pred[:,0]
    mu = y_pred[:,1]
    return tf.reduce_mean(-z*tf.math.log(mu) - (1-z)*tf.math.log(1-mu) - z*y*tf.math.log(lam) + z*tf.math.log(1-tf.exp(-lam)) + z*lam)

def hurdle_model(inpt_dim:int, hidden_widths:tuple):
    """Generates NN model for zero-inflated Poisson model. Expects output to contain 2 dimensions:
        the actual measured value, and an indicator value (1 if nonzero, 0 otherwise)."""
    inp = keras.layers.Input(inpt_dim)
    gate = GateLayer()
    item = gate(inp)
    for width in hidden_widths:
        item = keras.layers.Dense(width,activation='relu')(item)
    lam = keras.layers.Dense(1,activation='exponential')(item)  
    mu = keras.layers.Dense(1,activation='sigmoid')(item)
    #clip values to prevent overflow or underflow to 0 or 1, which diverges with the loss function
    out = keras.layers.Concatenate()([tf.clip_by_value(lam,1e-7,1e25),
                                      tf.clip_by_value(mu,1e-25,1-1e-7)])
    model = keras.Model(inputs=inp,outputs=out)
    model.compile(optimizer='adam',loss=poisson_hurdle_loss)
    return model

def get_loss_activation(loss_type,loss_fn,output_fn):
    if loss_type is None:
        return loss_fn,output_fn
    elif loss_type.lower() == 'poisson':
        return 'poisson','exponential'
    elif loss_type.lower() == 'bernoulli':
        return 'binary_crossentropy','sigmoid'
    elif loss_type.lower() == 'gaussian':
        return 'mse','linear'
    
def JGC_model(inpt_dim:int, hidden_widths:tuple,loss_type='poisson',loss_fn='poisson',output_fn='exponential'):
    ...
    loss_type (str): sets both the output neuron activation and loss function according to the loss type.
                     Options are 'poisson','hurdle','bernoulli','gaussian', None.
                     Set to None if you want to set the loss function and output neuron activation manually.
    loss_fn (str): for setting the loss function manually.
                   Note: this may require editing the preprocess_data() function.
                   Options are any string parseable by tensorflow as a loss function.
                   Ignored if loss_type is not None.
    output_fn (str): for setting the activation function of the output neuron manually.
                     Options are any string parseable by tensorflow as an activation function.
                     Ignored if loss_type is not None.
    """
    
    if loss_type is not None and loss_type.lower() == 'hurdle':
        return hurdle_model(inpt_dim,hidden_widths)
    
    assert loss_type is None or loss_type.lower() in ['poisson','bernoulli','gaussian']
    loss_fn,output_fn = get_loss_activation(loss_type,loss_fn,output_fn)
    ...
    for width in hidden_widths:
        item = keras.layers.Dense(width,activation='relu')(item)
    out = keras.layers.Dense(1,activation=output_fn)(item)        
    model = keras.Model(inputs=inp,outputs=out)
...
def infer_GC_one_parallel(target_index:int, model, data:np.ndarray, lam:float):
    """for parallelization"""
    X,y = preprocess_data(data,target_index,model.eta,model.contemp,model.standardize,
                          model.loss_type)
...
def infer_GC_all_parallel(model, data:np.ndarray, lam:float, processes=8):
    """Currently only supports one lam value for all target indices.
    """
    ...
    for target_index in range(model.dim):
        ...
        var_score_mat[target_index] = var_scores
...
class JGC():
    def __init__(self, dim:int, eta:int, n_iters=3, n_epochs=2000, batch_size=64,
                 hidden_widths=(50,50), sd_cutoff=1., contemp=False, loss_type='poisson',
                 loss_fn='poisson',output_fn='exponential', standardize=False):
        self.dim = dim
        self.eta = eta
        ...
        self.standardize = standardize #set to True only for continuous data
        
    def train_new(self, X:np.ndarray, y:np.ndarray, lam:float, target_index:int, seed=1):
        tf.random.set_seed(seed)
        model = JGC_model(X.shape[1], self.hidden_widths,
                          self.loss_type,self.loss_fn,self.output_fn)
        ...
        var_score, lag_score = self.get_scores(impt_score,target_index)
    ...
    def get_scores(self, impt_score:np.ndarray, target_index:int):
        if self.contemp: #contemporaneous vars exist: sum the impt_scores for time t and t-1
            ...
            lag_score = impt_score.reshape((self.eta,self.dim))
            var_score = np.max(lag_score,axis=0) #score for variable (ignoring lag)
    ...
    def infer_GC_one_target(self, data:np.ndarray, target_index:int, lam:float, verbose=True):
        X,y = preprocess_data(data,target_index,self.eta,self.contemp,self.standardize,
                              self.loss_type)
    ...
    def get_sign_one_target(self, target_index:int, iteration=0, threshold=False):
        ...
        var_score, lag_score = self.get_scores(meangrad,target_index)
